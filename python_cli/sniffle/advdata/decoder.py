...
ad_type_classes = {
    ...
    0x16: RemoteIDRecord,  # Mapping type 22 to RemoteIDRecord
    0x20: ServiceData32Record,
    0x21: ServiceData128Record,
    0xFF: decode_msd  # Assuming 0xFF is for manufacturer specific data
}
...
def decode_adv_data(data):
    ...
    while i < len(data):
        try:
            l = data[i]
            t = data[i+1]
            d = data[i+2:i+2+l]  # Corrected slicing to include the full data block
            if t in ad_type_classes:
                record_class = ad_type_classes[t]
                print(f"Processing type {t}, length {l}, data {d}")
                print(f"Instantiating record class for type {t}")
                records.append(record_class(t, d))
            else:
                print(f"No handler found for data type: {t}")
            i += 1 + l
